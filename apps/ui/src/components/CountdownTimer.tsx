...
const CountdownTimer: React.FC<CountdownTimerProps> = ({ ms, label }) => {
  ...
  const TimeUnit = ({ value, label }: { value: string, label: string }) => (
    <div className="flex flex-col items-center">
      ...
      <span className="text-[8px] uppercase tracking-widest text-zinc-600 mt-2 font-black italic">{label}</span>
    </div>
  );

  return (
    <div className="flex flex-col items-start gap-3">
      {label && <span className="text-[9px] font-black text-zinc-600 uppercase tracking-[0.4em] italic">{label}</span>}
