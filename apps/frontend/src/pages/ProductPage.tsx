...
import { getCreateOrderErrorMessage, useCreateOrder } from "../hooks/useCreateOrder";
...
function ProductPage() {
  ...
  const product = flashSaleInfo
    ? {
        ...
        images: flashSaleInfo.imageUrl ? [flashSaleInfo.imageUrl] : [],
      }
    : null;

  const availableStock = stockData?.availableStock ?? flashSaleInfo?.availableStock ?? 0;
  const isSoldOut = flashSaleInfo?.status === "active" && availableStock === 0;
  ...
  const handlePurchase = () => {
    ...
    if (!sku) return;
    if (effectiveStatus !== "active" || isSoldOut || availableStock <= 0) {
      return;
    }
  ...
  const nextImage = () => {
    setCurrentImageIndex((prev) => (allImages.length ? (prev + 1) % allImages.length : 0));
  };

  const prevImage = () => {
    setCurrentImageIndex((prev) =>
      allImages.length ? (prev - 1 + allImages.length) % allImages.length : 0
    );
  };
  ...
  if (productError && isApiError(productErrorObj) && productErrorObj.statusCode === 404) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center text-center px-4">
        <h2 className="text-3xl font-black mb-4 text-zinc-900">Product Not Found</h2>
        <Link
          to="/"
  ...
  if (productError && !flashSaleInfo) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center text-center px-4">
        <h2 className="text-3xl font-black mb-4 text-zinc-900">Could not load product</h2>
        <p className="text-zinc-600 mb-4">
          {productErrorObj instanceof Error ? productErrorObj.message : "Something went wrong."}
        </p>
        <Link
          to="/"
          className="text-yellow-600 font-bold hover:underline uppercase tracking-widest text-xs"
          ...
          <div className="aspect-square bg-zinc-50 border border-zinc-100 rounded-2xl sm:rounded-[2rem] lg:rounded-[2.5rem] overflow-hidden shadow-lg sm:shadow-xl relative group">
            ...
            {allImages.length > 1 && (
              <div className="absolute inset-0 flex items-center justify-between px-3 sm:px-6 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none sm:pointer-events-auto">
                <Button
                  ...
                  size="icon"
                  onClick={prevImage}
                  className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-white/90 backdrop-blur shadow-md text-zinc-900 hover:bg-white pointer-events-auto touch-manipulation"
            ...
            {(effectiveStatus === "ended" || isSoldOut) && (
              <div className="absolute inset-0 bg-white/20 backdrop-blur-[2px] flex items-center justify-center z-30">
                <span className="bg-zinc-900 text-white px-8 py-3 rounded-xl font-bold uppercase tracking-widest shadow-xl rotate-[-2deg] text-sm">
                  Sold Out
                </span>
              </div>
            )}
          ...
          <div className="bg-zinc-50 border border-zinc-100 rounded-2xl sm:rounded-[2rem] p-5 sm:p-6 lg:p-10 mb-8 sm:mb-10 space-y-6 sm:space-y-8 lg:space-y-10 shadow-sm">
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 sm:gap-6">
              ...
              {effectiveStatus !== "ended" &&
                ...
                timeRemaining > 0 && (
                  <CountdownTimer
                    ms={timeRemaining}
                    label={effectiveStatus === "upcoming" ? "Starts In" : "Ends In"}
                  />
                )}
            </div>
              ...
              <div className="bg-yellow-400/10 border border-yellow-400/20 p-4 sm:p-5 rounded-xl sm:rounded-2xl flex items-start sm:items-center gap-3 sm:gap-4">
                ...
                <div>
                  ...
                  <p className="text-zinc-700 text-[10px] uppercase font-bold tracking-widest mt-0.5">
                    Stock: {remainingStock} left • Limit: {flashSaleInfo.limitPerUser} per customer
                  </p>
                </div>
              </div>
            )}
            ...
            {!createOrderMutation.isPending && createOrderMutation.data && (
              <Alert className="border-green-500/50 bg-green-50 text-green-900 dark:bg-green-950 dark:text-green-100">
                ...
                <AlertDescription>Order ID: {createOrderMutation.data._id}</AlertDescription>
              </Alert>
            )}
            ...
            {hasPurchased && !createOrderMutation.data && (
              <Alert className="border-green-500/50 bg-green-50 text-green-900 dark:bg-green-950 dark:text-green-100">
                <AlertTitle>You've already purchased this item!</AlertTitle>
                {myOrder?._id && <AlertDescription>Order ID: {myOrder._id}</AlertDescription>}
              </Alert>
            )}
            ...
            <div className="flex flex-col gap-4">
              {effectiveStatus === "ended" || isSoldOut ? (
                <Button
                  type="button"
                  disabled
                  className="w-full min-h-[52px] py-5 sm:py-6 rounded-2xl font-bold text-lg uppercase tracking-widest italic bg-zinc-200 text-zinc-600"
                >
                ...
                <Button
                  ...
                  disabled
                  variant="secondary"
                  className="w-full min-h-[52px] py-5 sm:py-6 rounded-2xl font-bold text-lg uppercase tracking-widest italic bg-green-100 text-green-700 border-green-200"
                >
                  Already Purchased ✓
                </Button>
              ) : (
                <>
                  {isAuthenticated ? (
                    <Button
                      ...
                      className={`w-full min-h-[52px] py-5 sm:py-6 rounded-2xl font-black text-lg sm:text-xl uppercase tracking-widest shadow-xl active:scale-[0.98] italic touch-manipulation ${
                        effectiveStatus === "active" &&
                        !createOrderMutation.isPending &&
                        !isSoldOut &&
                        !hasPurchased &&
                    ...
                    <LoginModal
                      buttonProps={{
                        ...
                        className:
                          "w-full min-h-[52px] py-5 sm:py-6 rounded-2xl font-black text-lg sm:text-xl uppercase tracking-widest shadow-xl active:scale-[0.98] italic touch-manipulation bg-zinc-900 text-white hover:bg-black shadow-zinc-900/10",
                      }}
                    />
                  )}
                  <Button
                    type="button"
                    variant="outline"
                    className="w-full min-h-[48px] py-4 rounded-2xl font-bold text-zinc-700 text-[10px] uppercase tracking-widest italic touch-manipulation"
                  >
                    Add to Wishlist
                  </Button>
                </>
                ...
                <div>
                  ...
                  <div className="bg-zinc-50 border border-zinc-100 rounded-xl sm:rounded-2xl p-4 sm:p-5 lg:p-6 space-y-0">
                    ...
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-1 py-3 sm:py-2.5">
                      ...
                      <span className="text-sm text-zinc-900 font-black sm:text-right">iOS 18</span>
                    </div>
                  </div>
                </div>
              </div>
